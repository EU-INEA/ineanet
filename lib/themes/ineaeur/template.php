 ...
 */
function ineaeur_menu_tree__main_menu($variables) {
 ...
 */
function ineaeur_preprocess_page(&$variables) {
  // Bottom region.
  ...
  elseif (isset($variables['node']) && $variables['node']->type == "inea_cef_project") {
    $variables['page_type'] = 'inea_cef_project';
    $node = $variables['node'];
    if (!empty($node->field_inea_cef_transport_mode)) {
      $tid = $node->field_inea_cef_transport_mode[LANGUAGE_NONE][0]['tid'];
    }
    elseif (!empty($node->field_inea_cef_ener_corridor)) {
      $tid = $node->field_inea_cef_ener_corridor[LANGUAGE_NONE][0]['tid'];
    }
    $term_image = taxonomy_term_load($tid);
    $tid = $node->field_inea_cef_action_sector2[LANGUAGE_NONE][0]['tid'];
    $project_type = taxonomy_term_load($tid);
    if (!empty($project_type)) {
      $type = str_replace(" ", "-", drupal_strtolower($project_type->name));
      $variables['page_type'] = $variables['page_type'] . "-" . $type;
  ...
  if (!empty($term_image) && isset($term_image->field_term_image[LANGUAGE_NONE][0]['uri'])) {
    ...
    $path = _ineaeur_generate_term_path($term_image);
 ...
 */
function ineaeur_preprocess_menu_block_wrapper(&$variables) {
 ...
 */
function ineaeur_preprocess_taxonomy_term(&$variables) {
  $variables['page'] = TRUE;
}
 ...
 */
function ineaeur_preprocess_views_view(&$variables) {
  if (isset($variables['view']->name)) {
    $function = '_ineaeur_preprocess_views_view__' . $variables['view']->name;
    if (function_exists($function)) {
      $function($variables);
    }
    $function = '_ineaeur_preprocess_views_view__' . $variables['view']->name . '__' . $variables['view']->current_display;
    if (function_exists($function)) {
      $function($variables);
    }
  }
 ...
 */
function _ineaeur_preprocess_views_view__latest_news__block(&$variables) {
 ...
 */
function _ineaeur_preprocess_views_view__cef_energy_countries(&$variables) {
  $term = _ineaeur_generate_view_term($variables['view']);
 ...
 */
function _ineaeur_preprocess_views_view__ten_t_projects_by_programme_and_other_tag(&$variables) {
  $term = _ineaeur_generate_view_term($variables['view']);
  $variables['header'] = $term . $variables['header'];
}
 ...
 */
function _ineaeur_generate_view_term($view) {
 ...
 */
function _ineaeur_generate_term_path($term) {
 ...
 */
function ineaeur_preprocess_node(&$variables) {
 ...
 * Implements theme_preprocess_node().
 */
function ineaeur_preprocess_field(&$variables, $hook) {
  ...
  if ($element['#field_name'] == 'field_tag_priority_projects') {
    foreach ($variables['items'] as $id => $item) {
      ...
      $href = _ineaeur_generate_term_path($term);
      $new_item = array(
        '#type' => 'link',
 ...
 */
function ineaeur_preprocess_block(&$variables) {
 ...
 */
function ineaeur_menu_link__main_menu(array $variables) {
  $element = $variables['element'];
  $dropdown = '';
 ...
 */
function ineaeur_form_alter(&$form, &$form_state, $form_id) {
